...
def get_test_data():
    ...
    try:
        ...
        cursor.execute("SELECT Id_Tkt as id_ticket, Titulo as titulo, Id_Estado as id_estado, Id_Usuario_Asignado as id_asignado FROM tkt LIMIT 1")
        ticket = cursor.fetchone()
        
        # Obtener un usuario
        cursor.execute("SELECT idUsuario as id_usuario, Email as email FROM usuario LIMIT 1")
        usuario = cursor.fetchone()
        
        # Obtener estados
        cursor.execute("SELECT Id_Estado as id_estado, TipoEstado as nombre FROM estado LIMIT 5")
