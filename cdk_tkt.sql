...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`cdk_tkt` /*!40100 DEFAULT CHARACTER SET latin1 */;
...
insert  into `rol_permiso`(`idRol`,`idPermiso`) values (1,1),(1,2),(1,4),(1,7),(1,10),(1,11),(1,12),(1,13),(1,14),(1,15),(1,16),(2,3),(2,5),(2,8),(2,9),(2,10),(2,13),(2,15),(3,1),(3,3),(3,5),(3,8),(3,9),(3,10),(10,1),(10,2),(10,4),(10,5),(10,6),(10,7),(10,10),(10,11),(10,12),(10,13),(10,14),(10,15),(10,16),(11,1),(12,1);
...
insert  into `tkt`(`Id_Tkt`,`Id_Estado`,`Date_Creado`,`Date_Cierre`,`Date_Asignado`,`Date_Cambio_Estado`,`Id_Usuario`,`Id_Usuario_Asignado`,`Id_Empresa`,`Id_Perfil`,`Id_Motivo`,`Id_Sucursal`,`Habilitado`,`Id_Prioridad`,`Contenido`,`Id_Departamento`) values (1,7,'2025-11-18 14:19:44',NULL,'2025-11-18 14:25:47','2025-12-04 11:08:26',1,2,0,4,6,2,0,1,'esto es una prueba',21),(2,1,'2025-11-21 11:35:49',NULL,'2025-11-21 11:36:12',NULL,1,3,0,4,39,10,1,7,'Esto es una prueba Beta',59),(3,7,'2025-11-21 11:59:22',NULL,'2025-11-21 11:59:32','2025-11-21 12:03:36',1,3,0,4,6,1,1,2,'Prueba de seguimiento',66),(4,2,'2025-11-21 12:33:30',NULL,'2025-11-21 14:35:53','2025-11-21 13:52:28',1,3,0,4,5,1,1,3,'aaaaaaaaaaaaaaaaaaaa sssssssss',59),(5,2,'2025-11-21 14:36:17',NULL,NULL,'2025-12-04 11:18:03',1,1,0,4,8,2,1,2,'',66),(6,2,'2025-12-04 10:29:13',NULL,NULL,'2025-12-04 11:17:00',1,1,0,4,18,2,1,2,'Esto es un test',13);
...
insert  into `tkt_usuario_rol`(`idUsuario`,`id_rol`) values (1,2),(2,2),(3,3);
...
BEGIN
  ...
  SET p_resultado = 'Actualizado exitosamente';
END */$$
DELIMITER ;
...
BEGIN
  ...
  SET p_resultado = 'Agregado exitosamente';
END */$$
DELIMITER ;
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_asignar_ticket`(IN w_Id_Tkt BIGINT, IN w_Id_Usuario_Asignado INT)
BEGIN
  UPDATE tkt
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_eliminar_ticket`(IN w_id_ticket INT)
BEGIN
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_listar_tkts`(
    ...
    IN w_Id_Usuario INT(20),
    IN w_Nombre_Usuario VARCHAR(150), -- Nuevo parámetro para el nombre del usuario
    IN w_Id_Empresa INT(20),
    IN w_Id_Perfil INT(20),
    IN w_Id_Motivo INT(20),
    IN w_Id_Sucursal INT(20),
...
BEGIN
    DECLARE OFFSET INT;
    SET OFFSET = (w_Page - 1) * w_Page_Size;
    ...
    IF w_Id_Tkt IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND t.Id_Tkt = ", w_Id_Tkt);
    END IF;
    IF w_Id_Estado IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND t.Id_Estado = ", w_Id_Estado);
    END IF;
    IF w_Date_Creado IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND date(t.Date_Creado) = '", w_Date_Creado, "'");
    END IF;
    IF w_Date_Cierre IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND t.Date_Cierre <= '", w_Date_Cierre, "'");
    END IF;
    IF w_Date_Asignado IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND t.Date_Asignado >= '", w_Date_Asignado, "'");
    END IF;
    IF w_Date_Cambio_Estado IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND t.Date_Cambio_Estado <= '", w_Date_Cambio_Estado, "'");
    END IF;
    IF w_Id_Usuario IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND t.Id_Usuario = ", w_Id_Usuario);
    END IF;
    IF w_Nombre_Usuario IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND u.nombre LIKE '%", w_Nombre_Usuario, "%'");
    END IF;
    IF w_Id_Empresa IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND t.Id_Empresa = ", w_Id_Empresa);
    END IF;
    IF w_Id_Perfil IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND t.Id_Perfil = ", w_Id_Perfil);
    END IF;
    IF w_Id_Motivo IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND t.Id_Motivo = ", w_Id_Motivo);
    END IF;
    IF w_Id_Sucursal IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND t.Id_Sucursal = ", w_Id_Sucursal);
    END IF;
    IF w_Habilitado IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND t.Habilitado = ", w_Habilitado);
    END IF;
    IF w_Id_Prioridad IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND t.Id_Prioridad = ", w_Id_Prioridad);
    END IF;
    IF w_Contenido IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND t.Contenido LIKE '%", w_Contenido, "%'");
    END IF;
    IF w_Id_Departamento IS NOT NULL THEN
        SET @base_query = CONCAT(@base_query, " AND t.Id_Departamento = ", w_Id_Departamento);
    END IF;
    -- Agregar paginación
    SET @final_query = CONCAT(@base_query, " LIMIT ", OFFSET, ", ", w_Page_Size);
...
BEGIN
  IF p_comentario IS NULL OR LENGTH(TRIM(p_comentario)) = 0 THEN
    SELECT 0 success, 'Comentario vacío' mensaje;
  ELSE
...
proc: BEGIN
    ...
    -- En MySQL 5.5 simplemente copiamos el texto recibido (sin validación JSON)
    SET v_meta = p_meta_json;
    -- Bloquear fila del ticket
    ...
    IF v_estado_from IS NULL THEN
        SELECT 0 success, 'Ticket no encontrado' message, NULL nuevo_estado, NULL id_asignado; LEAVE proc;
    END IF;
    -- Comentario obligatorio para toda transición
    IF p_comentario IS NULL OR TRIM(p_comentario) = '' THEN
        SELECT 0 success, 'Comentario requerido' message, NULL nuevo_estado, NULL id_asignado; LEAVE proc;
    END IF;
    -- Buscar regla
    ...
    IF v_requiere_propietario IS NULL AND v_permiso IS NULL THEN
        SELECT 0 success, 'Transición no permitida' message, NULL nuevo_estado, NULL id_asignado; LEAVE proc;
    END IF;
    IF v_requiere_propietario = 1 AND (v_asignado_actual IS NULL OR v_asignado_actual <> p_id_usuario_actor) THEN
        SELECT 0 success, 'Solo el asignado puede realizar esta transición' message, NULL nuevo_estado, NULL id_asignado; LEAVE proc;
    END IF;
    -- Manejo de aprobación (placeholder simple)
    IF v_requiere_aprob = 1 THEN
        ...
        -- Aquí podrías forzar p_estado_to a un estado "Necesita Aprobación" si difiere
    END IF;
    ...
    UPDATE tkt
       SET Id_Estado = p_estado_to,
           ...
           -- updated_at / version: columnas no existen en 'tkt'; se agregarán aparte si se decide
